...
### ========== /sleeper set_user ==========
@tree.command(
    name="sleeper_set_user", description="Link a Discord user to a Sleeper user ID"
)
@app_commands.describe(discord_user="Discord user", sleeper_user_id="Sleeper user ID")
async def set_user(
    interaction: discord.Interaction, discord_user: discord.Member, sleeper_user_id: str
):
...
### ========== /sleeper set_league ==========
@tree.command(
    name="sleeper_set_league", description="Set default Sleeper league for this channel"
)
@app_commands.describe(league_id="Sleeper league ID")
async def set_league(interaction: discord.Interaction, league_id: str):
    league = sleeper_api.get_league(league_id)
...
### ========== /sleeper standings ==========
@tree.command(
    name="sleeper_standings", description="Show standings for this channel's league"
)
...
### ========== /sleeper matchup ==========
@tree.command(name="sleeper_matchup", description="Show matchups for a week")
@app_commands.describe(week="Week number")
async def matchup(interaction: discord.Interaction, week: int):
    channel_id = str(interaction.channel_id)
    if channel_id not in league_settings:
    ...
    for ids in pairs.values():
        if len(ids) == 2:
            ...
            msg += (
                f"> {u1}'s Starters: {', '.join(player_names(lineups.get(r1, [])))}\n"
            )
            msg += (
                f"> {u2}'s Starters: {', '.join(player_names(lineups.get(r2, [])))}\n"
            )
...
### ========== /sleeper my_team ==========
@tree.command(name="sleeper_my_team", description="Show your team starters")
async def my_team(interaction: discord.Interaction):
    ...
    await interaction.response.send_message("Couldn't find your team in this league.")
